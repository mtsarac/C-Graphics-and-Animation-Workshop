...
glm::vec2 getRandomOffset(float time) {
    ...
    float x = (sin(-time * 1.5f) * 0.5f + 1.0f) * 0.5f * 2.0f - 1.0f;
    float y = (cos(time * 2.5f) * 0.5f + 1.0f) * 0.5f * 2.0f - 1.0f;
...
void draw(Object obj) {
    ...
    glm::vec2 randomOffset = getRandomOffset(time);
    GLuint randomLoc = glGetUniformLocation(obj.program, "randomOffset");
    glUniform2f(randomLoc, randomOffset.x, randomOffset.y);
